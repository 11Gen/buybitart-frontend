...
// upd
  ...
  const onSubmit = async (data) => {
    ...
    if (typePay === "crypto" && cryptoType.token === "BTC") await PayBTC(fullData);
    else if (typePay === "crypto" && cryptoType.token === "TRC20") await PayUSDT(fullData, btcPrice);
              ...
              {(cart || []).map((item, index) => (
                ...
                >
                  ...
                  <span className="text-[#FCCB00]">
                    {typePay === "crypto"
                      ? cryptoType.token === "BTC" ? getPrice(item.price, 4, "BTC") : getPrice(btcPrice * item.price, 2, "USD")
                      : typePay === "card"
              ...
              <div className="w-full h-auto flex flex-col gap-1">
                <div className="font-main text-base font-[300] tracking-wide w-full flex justify-between items-center">
                  ...
                  <span className="text-white font-[500]">
                    {typePay === "crypto"
                      ? cryptoType.token === "BTC" ? totalPrice.toFixed(4) + "BTC" : (btcPrice * totalPrice).toFixed(2) + "USD"
                      : typePay === "card"
