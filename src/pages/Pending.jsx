...
const Pending = () => {
  ...
  useEffect(() => {
    const checkPaymentStatus = async () => {
      try {
        const response = await axios.get(`${apiUrl}/payment/status/${searchParams.get("orderId")}`);

        if (response.data.status === "completed") navigate(`/success?orderId=${searchParams.get("orderId")}`);
        else if (response.data.status === "canceled") navigate(`/error?orderId=${searchParams.get("orderId")}`);
      } catch (error) {
