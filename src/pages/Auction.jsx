...
const Auction = () => {
  ...
  const fetchAuctionData = useCallback(async () => {
    try {
      const [auctionResponse, bidsResponse] = await Promise.all([
        ...
        axios.get(`${import.meta.env.VITE_DB_LINK}/api/auctions/${hash}/bids?page=1&limit=5`),
      ]);
  ...
  const isTopBidder = useMemo(() => {
    ...
    return bids[0].user._id === userStore.user._id || bids[0].user._id === userStore.user.id;
  }, [bids, userStore.user]);
